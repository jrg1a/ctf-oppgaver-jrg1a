# Arkivportalen

**Kategori:** Password Forensics
**Poeng:** 250
**Type:** Statisk ZIP og lokal HTML
**Vanskelighetsgrad:** Medium

---

## Scenario

Et lite standteam hos Nordverk har sendt over en kopi av et gammelt
arkivområde og en passordliste de brukte under riggingen av demoen.

Arkivet er pakket i en passordbeskyttet ZIP. Inne i arkivet ligger en lokal
statusportal som skal åpnes i nettleseren, men portalen krever en dagskode.

Finn dagskoden, lås opp portalen og hent flagget.

---

## Vedlegg

- [`standarkiv.zip`](dist/standarkiv.zip), Passordbeskyttet arkiv
- [`passordliste.txt`](dist/passordliste.txt), Liten lekket passordliste

---

## Flaggformat

```
CTF{...}
```

---

## Hints

| Kostnad | Hint |
|---------|------|
| 20 poeng | ZIP-passordet er ikke tilfeldig. Bruk passordlisten som følger med. |
| 40 poeng | Klassiske ZIP-cracking-verktøy kan bruke en liten ordliste uten å brute-force hele nøkkelrommet. |
| 60 poeng | Etter utpakking: les logger nøye. Debug-linjer kan inneholde kodet informasjon. |
| 80 poeng | Portalen forventer dagskoden i normalisert form, ikke nødvendigvis slik den først dukker opp i loggen. |

---

## Løsningsvei (kun for arrangør)

Dette er en statisk kjede med ZIP-cracking, enkel decoding og lokal HTML.
Oppgaven skal ikke kreve container eller nettverk.

### Steg 1 — Crack ZIP-passordet

Bruk vedlagt wordlist:

```bash
zip2john standarkiv.zip > hash.txt
john --wordlist=passordliste.txt hash.txt
```

Riktig passord:

```text
Nordverk2026!
```

Alternativt kan man teste listen direkte med `fcrackzip` hvis det er
tilgjengelig.

### Steg 2 — Pakk ut og finn dagskoden

```bash
unzip standarkiv.zip
rg -n "kode|b64|debug|SKIFT" .
```

I `logger/hendelser.log` ligger:

```text
dagskode_b64=U0tJRlQtTk9SRExZUw==
```

Base64-dekod:

```bash
echo 'U0tJRlQtTk9SRExZUw==' | base64 -d
```

Dette gir:

```text
SKIFT-NORDLYS
```

### Steg 3 — Normaliser koden

`notater/kodepraksis.txt` sier at portalen bruker delen etter `SKIFT-`, i
små bokstaver. Dagskoden som skal skrives inn er:

```text
nordlys
```

### Steg 4 — Åpne portalen

Åpne `portal/portal.html` lokalt i nettleseren og skriv inn `nordlys`.
Portalen dekrypterer flagget i JavaScript.

Man kan også validere uten nettleser ved å bruke samme XOR-logikk mot
ciphertext-arrayet i HTML-en.

**ZIP-passord:** `Nordverk2026!`

**Dagskode:** `nordlys`

**Flagg:** `CTF{zip_j0hn_b64_portal}`
